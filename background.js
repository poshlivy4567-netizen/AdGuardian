...
chrome.runtime.onInstalled.addListener(async (details) => {
  if (details.reason === "install") {
    await chrome.storage.local.set({ enabled: true, cosmeticEnabled: true, allowlist: [] });
  }
  await setup();
});
...
async function getDashboard() {
  const [data, tab] = await Promise.all([
    chrome.storage.local.get({
      enabled: true, cosmeticEnabled: true, allowlist: [], blockedTotal: 0,
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  (async () => {
    ...
    if (msg.type === "setCosmetic") {
      await chrome.storage.local.set({ cosmeticEnabled: Boolean(msg.enabled) });
      return { ok: true };
    }

    if (msg.type === "getCosmeticState") {
      const { enabled, cosmeticEnabled, allowlist } = await chrome.storage.local.get({ enabled: true, cosmeticEnabled: true, allowlist: [] });
      const host = normaliseHost(sender.tab && sender.tab.url ? sender.tab.url : "");
      return { enabled, cosmeticEnabled, allowed: Boolean(host && allowlist.includes(host)) };
    }
